class PveAuth {
	...
	constructor(pveAuthOptions) {
		...
		if (
			!PveAuth.#IsPropertyValid(this.#pveAuthOptions.username) &&
			!PveAuth.#IsPropertyValid(this.#pveAuthOptions.password) &&
			!PveAuth.#IsPropertyValid(this.#pveAuthOptions.host)
		) {
		...
		this.#pveAuthOptions.host = `https://${
			this.#pveAuthOptions.host
		}:8006/api2/json`;
	}
	...
	async #GenerateTicket() {
		...
		var ticketHasExpired =
			this.#pveAuthOptions.apiTicket.timeStamp + 7080 < new Date().getTime();
		...
		if (ticketHasExpired) {
			var response = await pveCurl.PveCurl.Post(
				...
				{
					username: this.#pveAuthOptions.username,
					password: this.#pveAuthOptions.password,
				},
				"",
				""
			);
			...
			this.#pveAuthOptions.apiTicket.csrfPreventionToken =
				ticketData.CSRFPreventionToken;
				this.#pveAuthOptions.apiTicket.ticket = ticketData.ticket;
			this.#pveAuthOptions.apiTicket.timeStamp = new Date().getTime();
			...
			this.#pveAuthOptions.password = pveAuthInfo.apiTicket.ticket;
		}
	}
